          ...
          {miniHeadings.map((heading, index) => (
            <React.Fragment key={index}>
              {index === 0 ? (
                ...
                <Link href="/" className="font-medium text-[16px] leading-[19.2px] text-black">
                  Home
                </Link>
              ) : index === 1 ? (
                // Second mini heading is dynamic with href based on the heading
                <Link href={`/${heading}`} className="font-medium text-[16px] leading-[19.2px] text-black">
                  {heading}
                </Link>
              ) : (
                // Last mini heading has color #FB2E86
                <span className="font-medium text-[16px] leading-[19.2px] text-[#FB2E86]">
                  {heading}
                </span>
              )}
